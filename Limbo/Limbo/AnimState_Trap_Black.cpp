#include "pch.h"
#include "AnimState_Trap_Black.h"

AnimState_Trap_Black::AnimState_Trap_Black()
{

}

AnimState_Trap_Black::~AnimState_Trap_Black()
{
	atlasImg.reset();
}

void AnimState_Trap_Black::Init()
{
	frame = 0;
	addDelta = 0.0f;
	std::wstring imgName(TEXT("trap.png"));
	atlasImg = AssetManager::GetInstance()->GetImage(imgName);
	AssetManager::GetInstance()->SetXMLData(XMLRect, "XML\\trap_Black.xml");
}
void AnimState_Trap_Black::Update(Gdiplus::Rect* rect, float Delta)
{

}

void AnimState_Trap_Black::Update(Gdiplus::Rect* rect, float Delta, bool action)
{
	if (!action)
	{
		rect->X = XMLRect[0].X;
		rect->Y = XMLRect[0].Y;
		rect->Width = XMLRect[0].Width;
		rect->Height = XMLRect[0].Height;
		return;
	}

	//addDelta += Delta;

	//if (addDelta > 0.01f)
	//{
	//	addDelta = 0;
	//}
	++frame;
	if (frame > XMLRect.size() - 1)
	{
		frame = XMLRect.size() - 1;
	}

	rect->X = XMLRect[frame].X;
	rect->Y = XMLRect[frame].Y;
	rect->Width = XMLRect[frame].Width;
	rect->Height = XMLRect[frame].Height;
}

void AnimState_Trap_Black::Begin()
{
}

void AnimState_Trap_Black::End()
{
}

std::weak_ptr<Gdiplus::Image> AnimState_Trap_Black::GetAtlasImg()
{
	return atlasImg;
}

